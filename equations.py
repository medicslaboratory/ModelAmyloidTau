...
def ODEsystem(t, y):
    ...
    # TODO Voir OneNote pour le dernier terme.
    dydt[3] = p.kappa_ABooABpo * y[2] * (1 + p.AP * p.delta_APop) \
              - (p.d_M2ABpo * y[12] + p.d_M2hatABpo * y[14]) * (y[3] / (y[3] + p.K_ABpo))

    # Glycogen synthase kinase-type 3 (GSK-3) (G)
    dydt[4] = p.kappa_IG * p.I - p.d_G * y[4]

    # tau proteins (tau)
    dydt[5] = (y[8] / p.N_0) * p.lambda_tau + p.kappa_Gtau * y[4] - p.d_tau * y[5]
    ...
    #         où p.W_A = 1
    dydt[9] = p.kappa_ABpoA * y[3] + p.kappa_TaA * y[17] - p.d_A * y[9]
    ...
    dydt[10] = p.kappa_FoM * (y[7] / (y[7] + p.K_Fo)) + p.lambda_ABpoM * (y[3] / (y[3] + p.K_ABpoM)) \
               - p.d_M * y[10]
    ...
    dydt[11] = dydt[10] * (p.beta / (p.beta + 1)) - p.kappa_TBM2 * (y[15] / (y[15] + p.K_TB)) * y[11]
    ...
    dydt[12] = dydt[10] * (1 / (p.beta + 1)) + p.kappa_TBM2 * (y[15] / (y[15] + p.K_TB)) * y[11]
    ...
    #  relation avec M^hat (= M_1^hat + M_2^hat).
    dydt[13] = p.kappa_PM1hat * (y[18] / (y[18] + p.K_P)) * (p.M1hateq - y[13]) - p.d_M1hat * y[13]

    # Anti-inflammatory macrophages (M_2^hat)
    # TODO: À discuter, voir note (OneNote)
    dydt[14] = p.kappa_TB * y[15] - p.d_M2hat * y[14]
    ...
    #         dydt[15] = p.kappa_M1TB * y[12] + p.kappa_M1hatTB * y[14] - p.d_TB * y[15] (graph ..._10)
    dydt[15] = p.kappa_M1TB * y[11] + p.kappa_M1hatTB * y[13] - p.d_TB * y[15]
    # dydt[15] = p.kappa_M2TB * y[12] + p.kappa_M2hatTB * y[14] - p.d_TB * y[15]
    ...
    #  p.kappa_M2I10 * y[12] / p.K_M2 -> p.kappa_M2I10 * y[12]
    #  Différence d'ordre de grandeur: (Cependant, ne change pas vrm les résultats).
    #  p.kappa_M2I10/p.K_M2 = 6.67e-3/0.017 = 0.3924 vs. Hao (1 cste) = 6.67e-3.
    dydt[16] = p.kappa_M2I10 * y[12] - p.d_I10 * y[16]
    ...
    # Avant : dydt[17] = p.kappa_M1Ta * y[11] / p.K_M1 + p.kappa_M1hatTa * y[13] / p.K_M1hat - p.d_Ta * y[17]
    # (modif: graph ..._11 vs ..._9)
    dydt[17] = p.kappa_M1Ta * y[11] + p.kappa_M1hatTa * y[13] - p.d_Ta * y[17]
